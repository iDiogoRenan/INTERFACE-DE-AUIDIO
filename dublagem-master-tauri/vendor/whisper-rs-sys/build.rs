...
fn main() {
    ...
    if cfg!(feature = "cuda") {
        ...
        let cuda_root = env::var("CUDA_PATH")
            ...
            .or_else(|_| env::var("CUDAToolkit_ROOT"));
        if let Ok(cuda_root) = cuda_root {
            ...
            config.define("CUDAToolkit_ROOT", &cuda_root);
            config.define("CMAKE_CUDA_COMPILER", nvcc.display().to_string());

            if cfg!(target_os = "windows") {
                let host_toolset = if target.contains("i686") {
                    "host=x86"
                } else {
                    "host=x64"
                };
                config.generator_toolset(format!("{host_toolset},cuda={cuda_root}"));
                config.define("CMAKE_VS_GLOBALS", format!("CudaToolkitDir={cuda_root}\\"));
            }
        }

        if !cfg!(target_os = "windows") {
            if let Ok(host_compiler) = env::var("NVCC_CCBIN") {
                config.define("CMAKE_CUDA_HOST_COMPILER", host_compiler);
            }
        }
