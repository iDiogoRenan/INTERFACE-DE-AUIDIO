...
fn phase8_chunked_inference_rejects_mixed_reference_batch() {
    ...
    let error = cuda_f32_pipeline().generate_tokens(&request).unwrap_err();
...
#[test]
fn phase8_long_form_auto_reuses_first_chunk_reference_for_followup_chunks() {
    let _guard = acquire_gpu_test_lock().unwrap();
...
#[test]
fn phase8_long_form_clone_preserves_original_reference_for_each_chunk() {
    let _guard = acquire_gpu_test_lock().unwrap();
    let pipeline = cuda_f32_pipeline();
    let bundle = ReferenceArtifactBundle::from_root(deterministic_reference_root()).unwrap();
    let case = bundle.case_by_id("det_auto_long_chunked").unwrap();
...
fn phase8_raw_reference_and_prebuilt_prompt_generate_identical_tokens() {
    ...
    raw_request.ref_audios = vec![Some(ReferenceAudioInput::from_path(ref_audio_path()))];
